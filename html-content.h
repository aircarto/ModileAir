...
const char WEB_DIV_PANEL_START[] PROGMEM = "<div class='panels'>"
...
const char WEB_PAGE_FOOTER[] PROGMEM = "<br/><br/>"
		...
		"&copy; AirCarto</a>&nbsp;&nbsp("
                "<a href='https://github.com/aircarto/MobileAir/labels/bug' target='_blank' rel='noreferrer'>" INTL_REPORT_ISSUE "</a>"
...
const char LTE_CONFIG_SCRIPT[] PROGMEM = "<script>\
function load_lte_list(){var x=new XMLHttpRequest();x.open('GET','/lte');x.onload = function(){if (x.status === 200) {document.getElementById('ltelist').innerHTML = x.responseText;}};x.send();}\
</script>";
