...
const char LTE_SHIELD_SET_FILE[] = "+UDWNFILE";
//const char LTE_SHIELD_SET_BYTES[] = "+UDWNBLOCK";
const char LTE_SHIELD_READ_FILE[] = "+URDFILE";
const char LTE_SHIELD_DELETE_FILE[] = "+UDELFILE";
const char LTE_SHIELD_REQUEST_SEND[] = "+UHTTPC";
const char LTE_SHIELD_SET_CA[] = "+USECMNG";
const char LTE_SHIELD_SECPRO[] = "+USECPRF";
...
LTE_Shield_error_t LTE_Shield::reset(void)
{
    ...
    if (err == LTE_SHIELD_ERROR_SUCCESS)
    {
        ...
        {
            beginSerial(LTE_SHIELD_DEFAULT_BAUD_RATE, _config, _rxPin, _txPin);
            setBaud(_baud);
            delay(200);
            beginSerial(_baud, _config, _rxPin, _txPin);
            err = at();
            delay(500);
        }
        return init(_baud, _config, _rxPin, _txPin);
    }
    return err;
}
...
LTE_Shield_error_t LTE_Shield::sendSMS(String number, String message)
{
    ...
    if (command != NULL)
    {
        ...
        err = sendCommandWithResponse(command, ">", NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT);
...
LTE_Shield_error_t LTE_Shield::init(unsigned long baud, uint32_t config, int8_t rxPin, int8_t txPin, LTE_Shield::LTE_Shield_init_type_t initType)
{
    ...
    if (initType == LTE_SHIELD_INIT_AUTOBAUD)
    {
        ...
        {
            return init(baud, config, rxPin, txPin, LTE_SHIELD_INIT_RESET);
        }
    }
    else if (initType == LTE_SHIELD_INIT_RESET)
    {
        ...
        {
            return init(baud, config, rxPin, txPin, LTE_SHIELD_INIT_AUTOBAUD);
        }
    }
...
LTE_Shield_error_t LTE_Shield::waitForResponsePrint(const char *expectedResponse, uint16_t timeout)
{
    unsigned long timeIn;
    boolean found = false;
    int index = 0;
...
LTE_Shield_error_t LTE_Shield::waitForResponsePrintBytes(const char *expectedResponse, uint16_t timeout)
{
    unsigned long timeIn;
    boolean found = false;
    int index = 0;
    ...
    while ((!found) && (timeIn + timeout > millis()))
    {
        if (hwAvailable())
        {
            char b = readByte();
            Debug.write(b);
            if ((char)b == expectedResponse[index])
            {
                if (++index == strlen(expectedResponse))
                {
                    found = true;
...
size_t LTE_Shield::hwPrintString(const String &s)
{
    if (_hardSerial != NULL)
    {
        return _hardSerial->print(s);
...
size_t LTE_Shield::hwWriteBytes(const uint8_t *b, size_t size)
{
    // for (int i = 0; i < size - 1; i++)
	// {
	// 	Debug.printf(" %02x", b[i]);
	// 	if (i == size - 2) 
	// 	{
	// 		Debug.printf("\n");
	// 	}
    ...
    if (_hardSerial != NULL)
    {
        return _hardSerial->write(b, size);
    }
#ifdef LTE_SHIELD_SOFTWARE_SERIAL_ENABLED
    else if (_softSerial != NULL)
    {
        return _softSerial->write(b, size);
    }
#endif
...
uint8_t LTE_Shield::readByte(void)
{
    char ret;

    if (_hardSerial != NULL)
    {
        ret = (uint8_t)_hardSerial->read();
    }
#ifdef LTE_SHIELD_SOFTWARE_SERIAL_ENABLED
    else if (_softSerial != NULL)
    {
...
void LTE_Shield::beginSerial(unsigned long baud, uint32_t config, int8_t rxPin, int8_t txPin)
{
    ...
    {
        _softSerial->begin(baud, config, rxPin, txPin, );
    }
#endif
    delay(100);
}
...
LTE_Shield_error_t LTE_Shield::autobaud(unsigned long desiredBaud)
{
    ...
    {
        beginSerial(LTE_SHIELD_SUPPORTED_BAUD[b++], _config, _rxPin, _txPin);
        setBaud(desiredBaud);
        delay(200);
        beginSerial(desiredBaud, _config, _rxPin, _txPin);
        err = at();
    }
    if (err == LTE_SHIELD_ERROR_SUCCESS)
    {
...
uint8_t *LTE_Shield::lte_calloc_byte(size_t num)
{
    return (uint8_t *)calloc(num, sizeof(uint8_t));
}
...
LTE_Shield_error_t LTE_Shield::setHost(int api, const char *host)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_HTTP) + 40);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
...
LTE_Shield_error_t LTE_Shield::setPort(int api, int port)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_HTTP) + 10);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
...
LTE_Shield_error_t LTE_Shield::setHeader(int api, const char *header)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_HTTP) + 100);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=%d,9,\"%s\"", LTE_SHIELD_HTTP, api, header);
...
LTE_Shield_error_t LTE_Shield::setJSON(const String &json)
{
    char *command;
    LTE_Shield_error_t err;

    int json_size = json.length();

    command = lte_calloc_char(strlen(LTE_SHIELD_SET_FILE) + 20);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=\"data.json\",%d", LTE_SHIELD_SET_FILE, json_size);

    err = sendCommandWithResponse(command, ">", NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT, AT_COMMAND);
    free(command);
...
// LTE_Shield_error_t LTE_Shield::setBytes(const uint8_t *data, size_t size)
// {
//     char *command;
//     LTE_Shield_error_t err;

//     command = lte_calloc_char(strlen(LTE_SHIELD_SET_FILE) + 30);
//     if (command == NULL)
//         return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
//     sprintf(command, "%s=\"databuffer.bin\",%d", LTE_SHIELD_SET_FILE, size);

//     Debug.println(command);

//     err = sendCommandWithResponse(command, ">", NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT, AT_COMMAND);
//     //hwWriteBytes(data,LEN_PAYLOAD_NBIOT);
//     hwWriteBytes(data, size);
...
LTE_Shield_error_t LTE_Shield::setBytes(const char *data)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=\"databuffer.bin\",%d", LTE_SHIELD_SET_FILE, strlen(data));
...
LTE_Shield_error_t LTE_Shield::readJSON(void)
{
    char *command;
    LTE_Shield_error_t err;
...
LTE_Shield_error_t LTE_Shield::readBytes(void)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=\"databuffer.bin\"", LTE_SHIELD_READ_FILE);
...
LTE_Shield_error_t LTE_Shield::readResponse(void)
{
    char *command;
    LTE_Shield_error_t err;
...
LTE_Shield_error_t LTE_Shield::deleteBytes(void)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=\"databuffer.bin\"", LTE_SHIELD_DELETE_FILE);
...
LTE_Shield_error_t LTE_Shield::sendPOSTRequest(int profile, const char *path)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_REQUEST_SEND) + 70);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=%d,4,\"%s\",\"response.txt\",\"data.json\",4", LTE_SHIELD_REQUEST_SEND, profile, path);
    err = sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT);
    free(command);
    return err;
}

LTE_Shield_error_t LTE_Shield::sendPOSTRequestByte(int profile, const char *path)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_REQUEST_SEND) + 100);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=%d,4,\"%s\",\"response.txt\",\"databuffer.bin\",2", LTE_SHIELD_REQUEST_SEND, profile, path);
    err = sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT);
    free(command);
    return err;
}

LTE_Shield_error_t LTE_Shield::setSSL(int api, int id)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=%d,6,1,%d", LTE_SHIELD_HTTP, api, id);
    err = sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT, AT_COMMAND);
    free(command);
    return err;
}

// LTE_Shield_error_t LTE_Shield::setCAroot(const char *caroot, const char *name)
// {
//     char *command;
//     LTE_Shield_error_t err;

//     Debug.println(caroot);
//     Debug.println(strlen(caroot));

//     int ca_size = strlen(caroot);

//     command = lte_calloc_char(strlen(LTE_SHIELD_SET_CA) + 100);
//     if (command == NULL)
//         return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
//     sprintf(command, "%s=1,0,\"%s\",%d", LTE_SHIELD_SET_CA, name, ca_size);
...
LTE_Shield_error_t LTE_Shield::setCAroot(const char *name, const char *file)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=1,0,\"%s\",\"%s\"", LTE_SHIELD_SET_CA, name, file);
...
LTE_Shield_error_t LTE_Shield::setSecProfile1(int id)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_SECPRO) + 10);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=%d,0,1", LTE_SHIELD_SECPRO, id);
    err = sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT, AT_COMMAND);
    free(command);
    return err;
}

LTE_Shield_error_t LTE_Shield::setSecProfile2(int id, const char *name)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=%d,3,\"%s\"", LTE_SHIELD_SECPRO, id, name);
    err = sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT, AT_COMMAND);
    free(command);
    return err;
}
...
LTE_Shield_error_t LTE_Shield::uploadCA(const String &cert, const char *name)
{
    char *command;
    LTE_Shield_error_t err;

    int cert_size = cert.length();

    command = lte_calloc_char(strlen(LTE_SHIELD_SET_FILE) + 20);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=\"%s.cert\",%d", LTE_SHIELD_SET_FILE, name, cert_size);
...
LTE_Shield_error_t LTE_Shield::setCArootBytes(const uint8_t *caroot, size_t size, const char *name)
{
    char *command;
    LTE_Shield_error_t err;
    ...
    command = lte_calloc_char(strlen(LTE_SHIELD_SET_CA) + 60);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=1,0,\"%s\",%d", LTE_SHIELD_SET_CA, name, size);

    Debug.println(command);

    err = sendCommandWithResponse(command, ">", NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT, AT_COMMAND);
    hwWriteBytes(caroot, size);
...
LTE_Shield_error_t LTE_Shield::uploadCABytes(const uint8_t *caroot, size_t size, const char *name)
{
    char *command;
    LTE_Shield_error_t err;

    command = lte_calloc_char(strlen(LTE_SHIELD_SET_FILE) + 60);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    sprintf(command, "%s=\"%s.cert\",%d", LTE_SHIELD_SET_FILE, name, size);
...
// LTE_Shield_error_t LTE_Shield::listCA(void)
// {
//     char *command;
//     LTE_Shield_error_t err;

//     command = lte_calloc_char(strlen(LTE_SHIELD_SET_FILE) + 10);
//     if (command == NULL)
//         return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
//     sprintf(command, "%s=3", LTE_SHIELD_SET_FILE);
